...
The residuals of the random factors no longer show structure. Soft bugs still show greater residual variance than do Streptomyces. 

Now i try to add fixed-effect variables to the slopes (but not to the Ps.strain/Inv.strain random factor, as those are confounded).
...
anova(M3, M4) # Invader as random slope, M3 better AIC 222.79, sig interaction 0.0079
anova(M5, M6) # mediainfusion as random slope, M5 better AIC 220.51, sig interaction 0.0189
AIC(M1, M3, M5) # go with M1 model.  Only M1 converges, and it has almost the same low AIC as M5 (both AIC=220.something)
